...
$notes = isset($_GET['notes']) ? trim($_GET['notes']) : 'Stock removed manually';
...
try {
    ...
    if ($product['quantity_in_stock'] < $quantity) {
        echo json_encode([
            'status' => 'error',
            'message' => "Not enough stock. Current quantity: {$product['quantity_in_stock']}"
        ]);
        exit;
    }
    ...
    $transaction_stmt->execute([
        ...
        'quantity' => -$quantity, // Negative value for removal
    ...
    echo json_encode([
        'status' => 'success',
        'message' => "Successfully removed $quantity units of {$product['name']}",
    ...
    error_log("Inventory remove stock error: " . $e->getMessage());
